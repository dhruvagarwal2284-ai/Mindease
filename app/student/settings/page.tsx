...
import { DataPermissionCard, PrivacyBadge, ReauthDialog } from "@/components/privacy";
...
export default function StudentSettingsPage() {
  ...
  const [reauthFor, setReauthFor] = useState<null | "identity" | "delete" | ConsentKey>(null);
  const [showExport, setShowExport] = useState(false);
  const [deleteConfirm, setDeleteConfirm] = useState(false);
  const [deleteWord, setDeleteWord] = useState("");
  const [resetConfirm, setResetConfirm] = useState<null | "seeded" | "empty">(null);
  ...
  const handleConsentChange = (key: ConsentKey, next: boolean) => {
    ...
    <div className="space-y-5">
      ...
      {tab === "profile" ? (
        <div className="space-y-4">
          <section className="card p-4 sm:p-5">
            ...
            <div className="rounded-2xl border border-teal-200 bg-teal-50 px-5 py-6 text-center">
              ...
              <p className="muted mt-1 text-xs">
                ...
                <span className="font-mono font-medium">{state.identity?.caseId ?? "—"}</span>{" "}
                instead of a name.
              </p>
            </div>
            <Button className="mt-3" onClick={() => setReauthFor("identity")}>
              Generate a new pseudonym
            </Button>
            <p className="muted mt-2 text-xs">
              Posts you already made keep the old handle — a new pseudonym is a fresh start,
              ...
              <ul className="space-y-2">
                {state.cases
                  ...
                  .map((c) => (
                    ...
                    >
                      <div>
                        <p className="text-sm font-medium text-navy-900">{c.concern}</p>
                        <p className="muted text-xs">
                          {c.mode} · case {c.caseId} · {c.status.replace("_", " ")}
                        </p>
                      </div>
                      <Link
          ...
          <section className="card p-4 sm:p-5">
            <SectionTitle title="Session" subtitle="Security on a shared or borrowed device." />
            <p className="text-sm text-navy-700">
              MindEase locks itself after 15 minutes of inactivity so nothing sensitive sits
              on screen. Unlocking loses nothing.
            </p>
      ...
      {tab === "privacy" ? (
        <section className="card p-4 sm:p-5">
          ...
          <div className="space-y-2.5">
            {STORAGE_ROWS.map((r) => (
              <div key={r.what} className="rounded-xl border border-navy-100 p-3.5">
                <div className="flex flex-wrap items-start justify-between gap-2">
                  <p className="font-medium text-navy-900">{r.what}</p>
                  <PrivacyBadge status={r.status} />
                </div>
                <dl className="mt-2 grid gap-1 text-sm sm:grid-cols-2">
                  <div className="flex gap-2">
                    <dt className="muted shrink-0">Stored:</dt>
                    <dd className="text-navy-800">{r.where}</dd>
                  </div>
                  <div className="flex gap-2">
                    <dt className="muted shrink-0">Visible to:</dt>
                    <dd className="text-navy-800">{r.who}</dd>
                  </div>
                </dl>
              </div>
            ))}
          </div>
          <Callout tone="info" icon="ℹ️" className="mt-4">
            This prototype keeps everything in your browser&rsquo;s local storage, which is
            the stand-in for the platform&rsquo;s protected on-device storage model. Nothing
            is uploaded to a server, because there is no server.
          </Callout>
        </section>
      ) : null}
      ...
      {tab === "consent" ? (
        <div className="space-y-4">
          <section className="card p-4 sm:p-5">
            <SectionTitle title="Your privacy controls" />
            <ul className="divide-y divide-navy-50">
              {consentKeys.map((k) => (
                <li key={k} className="flex items-center justify-between gap-3 py-2.5">
                  <span className="text-sm text-navy-800">{CONSENT_COPY[k].label}</span>
                  <Badge tone={state.consent[k] ? "mint" : "neutral"}>
                    {state.consent[k] ? "ON" : "OFF"}
                  </Badge>
                </li>
              ))}
            </ul>
          ...
          <section className="space-y-2.5">
            ...
            {consentKeys.map((k) => (
              <DataPermissionCard
                key={k}
                label={CONSENT_COPY[k].label}
                why={CONSENT_COPY[k].why}
                sensitive={CONSENT_COPY[k].sensitive}
                checked={state.consent[k]}
                onChange={(v) => handleConsentChange(k, v)}
              />
            ))}
          </section>
        </div>
      ...
      {tab === "notifications" ? (
        <section className="card p-4 sm:p-5">
          ...
          <div className="flex items-start justify-between gap-4 rounded-xl border border-navy-100 p-4">
            <div>
              <p className="font-medium text-navy-900">Send me notifications</p>
              <p className="muted mt-0.5 text-sm">{CONSENT_COPY.notifications.why}</p>
            </div>
            <Toggle
              checked={state.consent.notifications}
              onChange={(v) => setConsent("notifications", v)}
          ...
          <div className="mt-4">
            ...
            <p className="muted mt-1 text-sm">
              Previews never carry content. Somebody glancing at your lock screen learns
              nothing.
            </p>
        ...
        <div className="space-y-4">
          <section className="card p-4 sm:p-5">
            ...
            {state.blockedHandles.length === 0 ? (
              <p className="muted text-sm">
                You have not blocked anyone. Blocking is one-way and silent — they are never
                told.
              </p>
            ) : (
              <ul className="space-y-2">
                {state.blockedHandles.map((h) => (
                  <li
                    key={h}
                    className="flex items-center justify-between gap-3 rounded-xl border border-navy-100 p-3"
                  >
                    <span className="font-mono text-sm text-navy-800">{h}</span>
                    <Button size="sm" onClick={() => blockHandle(h)}>
                      Unblock
                    </Button>
                  </li>
                ))}
              ...
              <ul className="space-y-2">
                {hiddenPosts.map((p) => (
                  ...
                  >
                    ...
                    <Button size="sm" onClick={() => hidePost(p.id)}>
                      Unhide
                    </Button>
                  </li>
                ))}
        ...
        <section className="card p-4 sm:p-5">
          ...
          <div className="space-y-2.5">
            {(
              ...
              ] as { key: keyof A11ySettings; label: string; why: string }[]
            ).map((a) => (
              <div
                key={a.key}
              ...
              >
                <div>
                  ...
                  <p className="muted mt-0.5 text-sm">{a.why}</p>
                </div>
                <Toggle
                  checked={state.a11y[a.key]}
                  onChange={(v) => setA11y(a.key, v)}
                  label={a.label}
                />
              </div>
            ))}
          </div>
          <Callout tone="mint" icon="⌨️" className="mt-4">
            Keyboard navigation, visible focus rings and screen-reader labels are always on.
            They are not a mode you switch into.
          </Callout>
        </section>
      ) : null}
      ...
      {tab === "data" ? (
        <div className="space-y-4">
          <section className="card p-4 sm:p-5">
            <SectionTitle title="Export my data" subtitle="Everything of yours, as JSON." />
            <p className="text-sm text-navy-700">
              Your identity, consent settings, check-ins, journal, your own posts and
              replies, and your support requests.
            </p>
            <Button className="mt-3" onClick={() => setShowExport(true)}>
              Show my data
            </Button>
          </section>
          ...
          <section className="rounded-2xl border-2 border-urgent-200 bg-urgent-50/40 p-4 sm:p-5">
            ...
            <p className="text-sm text-navy-800">
              Erases your pseudonym, check-ins, journal, your own posts and replies, and your
              support requests from this device. There is no backup and no undo — that is the
              point.
            </p>
            <Button tone="urgent" className="mt-3" onClick={() => setReauthFor("delete")}>
              Delete my data
            </Button>
          </section>
        </div>
      ...
      {tab === "prototype" ? (
        <section className="rounded-2xl border-2 border-dashed border-navy-300 bg-white p-4 sm:p-5">
          <p className="mb-3 inline-block rounded-full bg-navy-900 px-3 py-1 text-xs font-semibold tracking-wide text-white uppercase">
            Prototype controls
          </p>
          <p className="muted text-sm">
            These would not exist in a real deployment. They are here so the failure states
            and the demo reset can be shown on demand.
          </p>

          <div className="mt-4 space-y-2.5">
            <div className="flex items-start justify-between gap-4 rounded-xl border border-navy-100 p-4">
              <div>
                <p className="font-medium text-navy-900">Simulate: AI unavailable</p>
                <p className="muted mt-0.5 text-sm">
                  Surfaces &ldquo;Support analysis is temporarily unavailable. Your private
                  journal remains available.&rdquo;
                </p>
              </div>
              <Toggle
            ...
            <div className="flex items-start justify-between gap-4 rounded-xl border border-navy-100 p-4">
              <div>
                <p className="font-medium text-navy-900">Simulate: offline</p>
                <p className="muted mt-0.5 text-sm">
                  Surfaces &ldquo;You&rsquo;re offline. Your private draft has been saved
                  locally.&rdquo;
                </p>
              </div>
              <Toggle
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            <Button onClick={() => setResetConfirm("seeded")}>
              Reset with demo history
            </Button>
            <Button onClick={() => setResetConfirm("empty")}>Reset to an empty account</Button>
            <Link
              href="/demo"
        ...
        action={
          reauthFor === "identity"
            ...
            : reauthFor === "delete"
              ...
              : `turning off ${
                  reauthFor ? CONSENT_COPY[reauthFor as ConsentKey]?.label.toLowerCase() : ""
                }`
        }
        onVerified={() => {
          if (reauthFor === "identity") {
            const next = regenerateIdentity();
            toast("New pseudonym", "success", `You are now ${next.handle}.`);
          } else if (reauthFor === "delete") {
            setDeleteConfirm(true);
          } else if (reauthFor) {
            setConsent(reauthFor as ConsentKey, false);
            toast("Consent withdrawn", "info");
          }
          setReauthFor(null);
        }}
      />
            ...
            <Button
              ...
              onClick={() => {
                navigator.clipboard
                  ...
                  .catch(() => toast("Could not copy", "warning", "Select the text instead."));
              }}
            >
              Copy to clipboard
            </Button>
      ...
      >
        <p className="muted mb-3 text-sm">
          A real deployment would offer this as a file download. This prototype shows it
          instead, so you can see exactly what an export contains.
        </p>
        <Textarea
          readOnly
          rows={16}
          ...
          <>
            ...
            <Button
              ...
              onClick={() => {
                ...
                setDeleteConfirm(false);
                setDeleteWord("");
                toast("Everything deleted", "info", "This device holds nothing of yours.");
              }}
            >
              Delete everything
            </Button>
      ...
      <Modal
        ...
        size="sm"
        title={
          resetConfirm === "seeded" ? "Reset with demo history?" : "Reset to an empty account?"
        }
        description={
          resetConfirm === "seeded"
        ...
        footer={
          <>
            ...
            <Button
              tone="primary"
              onClick={() => {
                if (resetConfirm) resetAll(resetConfirm);
      ...
      {tab === "blocked" && state.blockedHandles.length === 0 && hiddenPosts.length === 0 ? (
        <EmptyState
          icon="🌿"
