...
export default function JournalHomePage() {
  ...
  // 🔥 NAYA LOGIC: Demo ke liye har entry ko alag point banayenge
  // const graphData = useMemo(() => {
  //   // Sirf wahi entries lo jisme mood hai
  //   const validEntries = entries.filter((e) => e.mood !== undefined);

  //   // Har entry ko graph ke format mein map kar do
  //   return validEntries.map((entry) => {
  //     return {
  //       id: entry.id, // 🔥 YAHI MISSING THA! Isse red error theek ho jayega
  //       date: entry.createdAt || new Date().toISOString(),
  //       mood: entry.mood,
  //       tags: entry.tags || [],
  //     };
  //   }).sort((a, b) => a.date.localeCompare(b.date)); // Purane se naya sort karega graph ke liye
  // }, [entries]);
  // 🔥 FIREBASE ENTRIES SE GRAPH KA DATA BANAO
  const graphData = useMemo(() => {
    // Sirf wahi entries lo jisme mood hai
    const validEntries = entries.filter((e) => e.mood !== undefined);

    return validEntries.map((entry, index) => {
      // 🔥 FIX 1: Date ko sahi format (YYYY-MM-DD) mein cut karna zaroori hai!
      const rawDate = entry.createdAt || new Date().toISOString();
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.error(error);
      toast("Failed to save entry", "warning"); // "warning" kar diya taaki error na aaye
    }
    setIsSubmitting(false);
  };
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      console.error(error);
      toast("Failed to delete", "warning"); // "warning" kar diya
    }
  };
  ...
  // 🔥 Ab count graphData ki length se aayega!
  const checkInCount = graphData.length;
        ...
        >
          <div className="mb-3 flex flex-wrap items-end justify-between gap-2">
            <div>
              ...
              <p className="muted mt-0.5 text-sm">
                Cloud-synced check-ins. Not a score, not a rating.
              </p>
            </div>
            <Link
              href="/student/journal/trends"
          ...
          {checkInCount < 2 ? (
            ...
            <>
              {/* 🔥 GRAPH KO NAYA FIREBASE DATA PASS KIYA */}
              <MoodTimeline checkIns={graphData} />
              <p className="muted mt-2 text-xs">
                {pluralize(checkInCount, "day")} recorded in cloud. Only you can see this.
              </p>
            </>
          )}
        </section>
